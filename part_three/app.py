...
app.config['SQLALCHEMY_KEY'] = 'ihaveasecret'
...
@app.route('/')
def root():
    posts = Post.query.order_by(Post.created_at.desc()).limits(5).all()
...
def tags_new():
    ...
    posts = Post.query.filter(Post.id_(post_ids)).all()
